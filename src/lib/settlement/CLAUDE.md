...
- The chain context is only assembled when both parties agree on a chain **and** an escrow deployment exists for it. Without the contract there is no EIP-712 domain, and the payload carries a memo naming the gap rather than emitting a zero address.
- **Escrow contracts live in `settlement_chain_configs`, keyed on `(company, chain_id)`** — not in an env var. An escrow contract *is* a deployment on one chain; the same address elsewhere is a different contract, usually nothing at all, and a platform-wide address would route one tenant's money through another's contract. `SETTLEMENT_VERIFYING_CONTRACT` survives as a fallback for single-deployment installs.
- A missing escrow is a **memo, not a blocker**. The bank leg is complete and settleable on its own; refusing the whole payload because the optional on-chain route is unconfigured would withhold a working instruction over an absent one.

Records go through `/api/settlement/counterparty-finance` and `/api/settlement/chain-configs` on the **cookie client under RLS** — unlike `settlement_payloads`, which is generated and must never be user-editable, these are user data: somebody has to type the IBAN in. The UI is Settings → **Settlement & Banking** (`settlement-finance-settings.tsx`), which **imports the validators from `counterparty-finance.ts` rather than reimplementing them** — that module is pure precisely so it can run in both places, and a second client-side IBAN check would drift from the server's within a release.

## Agreement and the payload, in the UI

`claim-settlement-panel.tsx` on the claim workspace is the front end for the whole chain: which rule set computed the claim, the eligibility checklist, the agree action, and the generated instruction (legs, excluded components, blockers, missing fields, digest). It **reads** the payload from `settlement_payloads` and never generates one — generation belongs to the outbox consumer so exactly one document exists per agreed calculation, and a GET that generated on demand would mint a new one with a new `issuedAt` on every page view. `CRITERION_LABELS` lives in `clearinghouse.ts` beside the criteria it describes, and a test asserts every criterion has one.
