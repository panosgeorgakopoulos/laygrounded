...
`invitations.ts` is the other pure module here: token shape, the 7-day expiry, and `decideRedemption` — one total function covering the single path by which somebody joins a company they did not create. **The refusal ORDER is deliberate and is pinned by tests**, because it is invisible to the type system and changes what a real person is told to do: state before identity (an expired link is expired whoever holds it, and saying so does not confirm to a stranger that they hold a real address's token), identity before membership (mentioning the reader's own company to the wrong person leaks that their account has a tenant), and `ALREADY_MEMBER` before `USER_ALREADY_IN_ANOTHER_COMPANY` (a double-click is a shrug; a foreign company is a support ticket). `invitations-server.ts` holds the I/O — note that it writes the **membership before stamping the invitation accepted**, because there is no transaction across two PostgREST calls and a crash between them must leave a person who is in the company rather than a burnt token and no membership.
